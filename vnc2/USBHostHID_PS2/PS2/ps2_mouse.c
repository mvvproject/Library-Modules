...
void MS_cmd(unsigned char command, PS2_mouse_t* PS2_mouse) {
  ...
  case 0xFF: //reset
    ...
    while(PS2dev_write(0xAA)!=0x00);  
    while(PS2dev_write(0x00)!=0x00);
...
//	PS2_mouse->DataRepEN  = 0;
  ...
  case 0xE9: //status request
    ack();
    while(PS2dev_write(0xE6)!=0x00);					// ƒаем инфу о настройках мыши 
    while(PS2dev_write(PS2_mouse->Resolution)!=0x00);  // и если потребуетс€ запомненные от хоста параметры,
    while(PS2dev_write(PS2_mouse->SampleRate)!=0x00);	// если тот проверить захочет
    //      send_status();
    break;
